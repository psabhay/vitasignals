...
final class HealthDataStore: ObservableObject {
    ...
    func refresh() {
        ...
        // Reuse existing context when possible to preserve the row cache.
        // Only recreate when there is no context yet.
        let ctx: ModelContext
        if let existing = self.context {
            // Rollback any unsaved changes to pick up background writes
            existing.rollback()
            ctx = existing
        } else {
    ...
    func fetchRecords(from startDate: Date, to endDate: Date, metricTypes: Set<String>? = nil) -> [HealthRecord] {
        ...
        // When specific metric types are requested, fetch per-type to use
        // the database index on metricType and avoid loading unwanted records.
        if let types = metricTypes, !types.isEmpty {
            var combined: [HealthRecord] = []
            for type in types {
                var descriptor = FetchDescriptor<HealthRecord>(
                    predicate: #Predicate {
                        $0.metricType == type && $0.timestamp >= start && $0.timestamp < end
                    },
                    sortBy: [SortDescriptor(\.timestamp, order: .reverse)]
                )
                descriptor.fetchLimit = 10000
